        ...
        if(input.isKeyPressed(SDL_BUTTON_LEFT)) { // Answer clicked, check stuff...
            ...
            if(optionChosen != -1) {
                ...
                if(m_currentQuestion->answers[optionChosen].followingQuestion) {
                    m_currentQuestion = m_currentQuestion->answers[optionChosen].followingQuestion.get();
                    initConversation();
                }
            }
        }
...
QuestManager::QuestManager(std::string questionListPath, std::string flagListPath, std::string tradeListPath)
{
    ...
    for(int i = 0; i < m_questionList.size(); i++) {
        questions.push_back(m_questionList[i].get());
    }
    std::vector<Flag*> flags;
    for(int i = 0; i < m_flagList.size(); i++) {
        flags.push_back(m_flagList[i].get());
    }
    std::vector<TradeTable*> trades;
    for(int i = 0; i < m_tradeTables.size(); i++) {
        trades.push_back(m_tradeTables[i].get());
    }

    m_dialogueManager = std::make_unique<DialogueManager>(&questions, &flags, &trades);
}
...
void QuestManager::readDialogueFromList(std::string listPath) {
    ...
    if(file.fail()) {
        ...
        std::vector<Question*> d = getDialogue(file);
        for(unsigned int i = 0; i < d.size(); i++) {
            m_questionList.push_back(std::unique_ptr<Question>(d[i]));
        }
    }
}
...
void QuestManager::readTradesFromList(std::string listPath) {
    ...
    if(file.fail()) {
        ...
        m_tradeTables.clear();
        std::vector<TradeTable*> d = getTradeTables(file);
        for(unsigned int i = 0; i < d.size(); i++) {
            m_tradeTables.push_back(std::unique_ptr<TradeTable>(d[i]));
        }
    }
}

Question* QuestManager::readQuestion(std::vector<std::string> lines) {
    std::unique_ptr<Question> q = std::make_unique<Question>();
    {
        //int depth;
        q->str = lines[0];
            ...
            if (lines[i] == "#BEGIN") {
                ...
                if(j < newLines.size()-1) { // Lines left over
                    ...
                    if(extraLines.size() != 0) {
                        std::unique_ptr<Question> q = std::make_unique<Question>();
                        q.reset(readQuestion(extraLines));
                        a.followingQuestion = std::move(q);
                    }
                }
...
std::vector<std::unique_ptr<Flag>> QuestManager::getFlags(std::ifstream& file) {
    int pos = file.tellg();
    file.seekg(0, std::ios::end);
    int filesize = file.tellg();
    file.seekg(pos);
    ...
    std::string line;
    while(std::getline(file, line)) {
        std::unique_ptr<Flag> f = std::make_unique<Flag>(line);
...
std::vector<TradeTable*> QuestManager::getTradeTables(std::ifstream& file) {
    ...
    while(std::getline(file, line)) {
        ...
        if(line == "#BEGIN") {
            ...
            table = readTradeTable(lines, id++).get();
...
std::unique_ptr<TradeTable>& QuestManager::readTradeTable(std::vector<std::string> lines, unsigned int id) {
    std::unique_ptr<TradeTable> table = std::make_unique<TradeTable>();
...
Trade* QuestManager::readTrade(std::string line) {
    ...
    std::unique_ptr<Trade> t = std::make_unique<Trade>();
    std::unique_ptr<Item> reqItem = std::make_unique<Item>(reqQuan, reqId, false);
    std::unique_ptr<Item> rewItem = std::make_unique<Item>(rewQuan, rewId, false); /// TODO: Metadata

    t->requiredItem = reqItem.get();
    t->rewardedItem = rewItem.get();
