...
class QuestManager
{
    public:
        ...
        Question*               getQuestion(unsigned int index){ return m_questionList[index].get(); }
        unsigned int            getQuestionListSize()    const { return m_questionList.size(); }
    ...
    private:
        ...
        std::unique_ptr<DialogueManager> m_dialogueManager; /// TODO make this private again please :)
        GLEngine::GUI* m_gui = nullptr;
        ...
        std::unique_ptr<TradeTable>& readTradeTable(std::vector<std::string> lines, unsigned int id);
        Trade* readTrade(std::string line);

        std::vector<std::unique_ptr<Flag>> getFlags(std::ifstream& file);

        std::vector<std::unique_ptr<Question>> m_questionList; // Full list of questions (dialogue "heads") in game
        std::vector<std::unique_ptr<Flag>> m_flagList; // Full list of all flags that exist
        std::vector<std::unique_ptr<TradeTable>> m_tradeTables; // Full list of all trade tables that exist
};
