...
class EntityManager
{
    ...
    public:
        ...
        std::vector<Entity>* getAllEntities() {
            std::vector<Entity>* allEnts = new std::vector<Entity>();
            if(m_entities) {
                for(unsigned int i = 0; i < m_entities->size(); i++) {
                    allEnts->push_back((*m_entities)[i]);
                }
            }
            if(m_talkingEntities) {
    ...
    private:
        std::vector<Entity>* m_entities = nullptr; // All entities, excluding TalkingNPCs
        std::vector<TalkingNPC>* m_talkingEntities = nullptr;  // Pointers to talkingNpcs to preserve special data.
