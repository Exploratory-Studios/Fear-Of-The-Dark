...
void EntityManager::init(Player* player, std::vector<Entity*> entities, AudioManager* audioManager, Chunk* chunks[WORLD_SIZE]) {
    m_audioManager = audioManager;

    addPlayer(player, chunks);
    for(int i = 0; i < entities.size(); i++) {
        addNpc(entities[i], chunks);
    }
}
...
void EntityManager::addPlayer(Player* player, Chunk* chunks[WORLD_SIZE]) {
    m_player = player;

    int chunkIndex = (((int)(player->getPosition().x / TILE_SIZE) / CHUNK_SIZE) % WORLD_SIZE) - 1;
    chunks[chunkIndex]->addEntity(*player);
}

void EntityManager::addNpc(Entity* entity, Chunk* chunks[WORLD_SIZE]) {
    m_entities.push_back(entity);

    int chunkIndex = (((int)(entity->getPosition().x / TILE_SIZE) / CHUNK_SIZE) % WORLD_SIZE) - 1;
    chunks[chunkIndex]->addEntity(*entity);
}

void EntityManager::addNpc(glm::vec2 position, unsigned int id, Chunk* chunks[WORLD_SIZE]) {
    Entity* npc = new Entity(position, id, m_audioManager);

    m_entities.push_back(npc);

    int chunkIndex = (((int)(position.x / TILE_SIZE) / CHUNK_SIZE) % WORLD_SIZE) - 1;
    chunks[chunkIndex]->addEntity(*npc);
}

void EntityManager::addTalkingNpc(glm::vec2 position, unsigned int id, Chunk* chunks[WORLD_SIZE]) {
    TalkingNPC* npc = new TalkingNPC(position, id, m_audioManager);
