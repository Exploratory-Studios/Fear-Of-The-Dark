...
void GameAudioManager::init()
{
	...
	for(int i = 0; i < MORNING_MUSIC_LENGTH; i++) {
		std::string path = "Music/MorningMus" + std::to_string(i) + ".mus";
		m_music.push_back(BARE2D::ResourceManager::loadMusic(path));
	}
	for(int i = 0; i < DAY_MUSIC_LENGTH; i++) {
		std::string path = "Music/DayMus" + std::to_string(i) + ".mus";
		m_music.push_back(BARE2D::ResourceManager::loadMusic(path));
	}
	for(int i = 0; i < AFTERNOON_MUSIC_LENGTH; i++) {
		std::string path = "Music/AfternoonMus" + std::to_string(i) + ".mus";
		m_music.push_back(BARE2D::ResourceManager::loadMusic(path));
	}
	for(int i = 0; i < NIGHT_MUSIC_LENGTH; i++) {
		std::string path = "Music/NightMus" + std::to_string(i) + ".mus";
		m_music.push_back(BARE2D::ResourceManager::loadMusic(path));
	}
	for(int i = 0; i < NIGHTMARE_MUSIC_LENGTH; i++) {
		std::string path = "Music/NightmareMus" + std::to_string(i) + ".mus";
		m_music.push_back(BARE2D::ResourceManager::loadMusic(path));
	...
	*/
	...
	for(int i = 0; i < SOUND_EFFECT_LENGTH; i++) {
		std::string path = "Sounds/" + m_soundEffectFiles[i];
