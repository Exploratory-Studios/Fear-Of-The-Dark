...
bool InventoryBase::onDragDropItemRemoved(const CEGUI::EventArgs& e) {
	...
	if(removed_item->currentDropTargetIsValid())
		return false; // This means that it stays in the same receiver
...
bool InventoryBase::onDragDropItemAdded(const CEGUI::EventArgs& e) {
	...
	if(getItemIndex(item) != (unsigned int)-1)
		return false; // It's already in this receiver. Don't do anything.
...
void InventoryBase::draw(GLEngine::SpriteBatch& sb,
	...
	if(m_initedGUI && m_frameWindow->isVisible()) {
...

		{
			...
			for(unsigned int i = 0; i < m_items.size(); i++) {
				glm::vec2 screenCoords = glm::vec2(m_gridItems[i]->getGridBasePixelRect().getPosition().d_x,
												   m_gridItems[i]->getGridBasePixelRect().getPosition().d_y);
				glm::vec2 screenSize   = glm::vec2(m_gridItems[i]->getGridBasePixelRect().getWidth(),
												   m_gridItems[i]->getGridBasePixelRect().getHeight());
