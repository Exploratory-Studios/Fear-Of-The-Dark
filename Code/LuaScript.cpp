...
namespace ScriptingModule {
	...
	int LuaScript::loadScriptString(std::string script) {
		...
		// Now that the script is loaded, we need to put it in the registry and return that value
		int ref = luaL_ref(T, LUA_REGISTRYINDEX);
		if(ref == LUA_REFNIL) {
			Logger::getInstance()->log("LUA ERROR: REFNIL on new file load!", true);
		}
	...
	void LuaScript::run(unsigned int chunkIndex, std::vector<Argument>& args) {
		...
		if(args.size() > 0) {
			...
			//err = lua_pcall(T, 1, 0, 0);
			err = lua_resume(T, m_parent, 1);
		} else {
			//err = lua_pcall(T, 0, 0, 0);
			err = lua_resume(T, m_parent, 0);
		}
	...
	int LuaScript::l_getFlag(lua_State* L) {
		QuestModule::QuestManager* qm = static_cast<QuestModule::QuestManager*>(getUpvalue(L, QUESTMANAGER_KEY));
	...
	int LuaScript::l_waitForEvent(lua_State* L) {
		...
		return continuationFunc(L, lua_yieldk(L, 0, 0, continuationFunc), 0);
	}

	int LuaScript::continuationFunc(lua_State* L, int status, lua_KContext ctx) {
		lua_remove(L, 1); // Get rid of the eventType (on top of stack)

		return 1; // One argument: the arguments table.
	}

	int LuaScript::l_triggerEvent(lua_State* L) {
		// Get event type
		std::string eventType = lua_tostring(L, 1);
		...
		EventModule::EventQueue::triggerEvent(eventType, args);
	...
	int LuaScript::l_startQuest(lua_State* L) {
		QuestModule::QuestManager* qm = static_cast<QuestModule::QuestManager*>(getUpvalue(L, QUESTMANAGER_KEY));
