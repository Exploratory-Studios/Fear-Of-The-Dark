...
namespace ScriptingModule {
	...
	struct Argument {
		//Argument();
		Argument(std::string key, std::string val, bool isMetadata = false) :
			key(key), val(val), isMetadata(isMetadata) {
		}
		std::string key;
		std::string val;
	...
	class ScriptQueue {
	  ...
	  public:
		...
		static void activateScript(
			unsigned int		  id,
			std::vector<Argument> args =
				{}); // Starts a script using the id given earlier. Returns UUID of specific script (with arguments, etc.)
	  ...
	  private:
		...
		static std::vector<Script>
			m_activeScripts; // Is a list of active scripts' copies, with blank data such as place, timer info, etc.
	};
