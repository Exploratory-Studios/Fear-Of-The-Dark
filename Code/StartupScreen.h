...
class Text {
  public:
	Text(glm::vec2			   pos,
		...
		m_colour(GLEngine::ColourRGBA8(colour.r, colour.g, colour.b, 0.0f)), m_text(text), m_textScale(textScale),
...
class Image {
  public:
	Image(glm::vec2				pos,
		...
		m_pos(pos),
		m_startTime(startTime * 60.0f), m_endTime(endTime * 60.0f), m_fadeTime(fadeTime * 60.0f),
		m_colour(GLEngine::ColourRGBA8(colour.r, colour.g, colour.b, 0.0f)), m_filepath(filepath), m_size(size),
		m_splashScreen(splashScreen) {
	}
