...
class Tile {
	public:
		...
		void draw(GLEngine::SpriteBatch& sb, int xOffset, int depthDifference);
		...
		void onInteract_RightClicked() {
			if(m_interactScriptID_used != -1) ScriptingModule::ScriptQueue::activateScript(m_interactScriptID_used, generateLuaData());
		}
